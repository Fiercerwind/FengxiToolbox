...
def process_one_audio_file(job, convert_audio_format, copy_file_safe):
    ...
    if parallel_workers > 1:
        ...
        with executor_factory(max_workers=parallel_workers) as executor:
            for job in jobs:
                ...
                futures[executor.submit(process_one_audio_file, job, convert_audio_format, copy_file_safe)] = job
        ...
        for index, job in enumerate(jobs):
            ...
            _call(callbacks.on_item_started if callbacks else None, src, job[2])
            try:
                item = process_one_audio_file(job, convert_audio_format, copy_file_safe)
            except Exception as exc:
                item = {"src": src, "output": "", "status": "failed", "ok": False, "message": str(exc)}
            _call(callbacks.on_item_finished if callbacks else None, item)
            handle_item(item)
